...
int init_pipeline_pool(const char *dat_path, const RadarMeta *meta, PipelinePool *pool) {
    ...
    // 1. 원본 데이터 한 번에 로드 (가장 빠른 fread 사용)
    if (load_complex_bin_all_fread(dat_path, meta->num_pulses, meta->num_fast_time_samples, 232, &pool->raw_data) != 0) {
        return -1;
    }

    // 2. 3중 버퍼 메모리 할당 및 초기화
